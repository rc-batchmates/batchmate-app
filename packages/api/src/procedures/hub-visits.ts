...
export const hubVisits = server.hubVisits.handler(
	async ({ input, context }) => {
		if (!context.user) {
		...
		if (date > today) {
			throw new ORPCError("BAD_REQUEST", {
				message: "Cannot view hub visits in the future",
			})
		}

		const { data, error } = await context.recurseApi.GET("/hub_visits", {
			params: { query: { date, per_page: 200 } },
		})
		...
		// The overnight-stale check compares against the current hour, so it only
		// makes sense for today. Past days never offer check-in.
